...
var App = {
  ...
  _buildMainNav: function () {
    var nav = $('.site-nav');
    nav.background = nav.find('.submenu-backgroud');
    nav.background.forced = false;
    nav.activeItem = null;
    if (nav.background.hasClass('forced-visible')) {
      nav.background.forced = true;
      nav.activeItem = nav.find('.submenu.forced-visible');
    }
    nav.find('.main-item').each(function (item) {
      var menuItem = $(this);
      menuItem.subMenu = menuItem.find('.ui.submenu');
      menuItem.hover(function() {
        menuItem.subMenu.show();
        if (!nav.background.forced) {
          nav.background.show();
        } else if (menuItem.subMenu[0] != nav.activeItem[0]) {
          nav.activeItem.hide();
        }
      }, function(event) {
        var target = $(event.relatedTarget);
        if (!target.hasClass('subitem') && !target.hasClass('submenu')) {
...
+function ($) { "use strict";
  ...
  Zoom.prototype._triggerAnimation = function () {
    ...
    var imageCenterY = imageOffset.top + ($(this._targetImage).attr("height") / 2)
